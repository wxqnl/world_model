...
usage() {
  cat <<'EOF'
WM3D V8 从零入口（1B/5B 与数据源均由 profile 决定）：

  ./run_v8.sh env
  ./run_v8.sh lock-resolve <resolve_source_lock.py 参数...>
  ./run_v8.sh download <download_sources.py 参数...>
  ./run_v8.sh archive-collection <materialize_archive_collection.py 参数...>
  ./run_v8.sh external-convert <run_external_converter.py 参数...>
  ./run_v8.sh beta-task-list <list_agibot_beta_tasks.py 参数...>
  ...
  ./run_v8.sh cache-plan <plan_cache_tasks.py 参数...>
  ./run_v8.sh cache-worker <run_cache_worker.py 参数...>
  ./run_v8.sh cache-seal <seal_episode_cache.py 参数...>
  ./run_v8.sh window <materialize_window_index.py 参数...>
  ./run_v8.sh normalization <build_grouped_normalization.py 参数...>
  ./run_v8.sh runtime <materialize_runtime.py 参数...>
  ./run_v8.sh preflight <torchrun 参数...> -- --runtime <runtime.yaml>
  ./run_v8.sh train <torchrun 参数...> -- <训练参数...>
...
need_python() {
  [[ -x "${PYTHON_BIN}" ]] || {
    echo "找不到 ${PYTHON_BIN}；先运行 ./run_v8.sh env 或设置 PYTHON_BIN。" >&2
    exit 2
  }
}
...
torchrun_preflight_split() {
  ...
  "$(dirname "${PYTHON_BIN}")/torchrun" "${torch_args[@]}" \
    -m wm3d_v3.training.pretrain "${app_args[@]}" --preflight-only
}
...
need_python_unless_env() {
  [[ "${command}" == env || "${command}" == smoke-real || \
...
case "${command}" in
  env)
    [[ $# -eq 0 ]] || { usage; exit 2; }
    exec bash scripts/setup_environment.sh
    ;;
  ...
  train) torchrun_split wm3d_v3.training.pretrain "$@" ;;
  eval) torchrun_split wm3d_v3.training.offline_eval "$@" ;;
  smoke-real)
...
files = sorted(path for root in (Path("wm3d_v3"), Path("scripts"), Path("tests")) for path in root.rglob("*.py"))
for path in files:
    compile(path.read_bytes(), str(path), "exec")
print(f"compiled {len(files)} Python files")
PY
    bash -n run_v8.sh scripts/setup_environment.sh
