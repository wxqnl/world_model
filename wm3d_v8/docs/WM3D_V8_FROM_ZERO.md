...
本文给出空服务器到训练 checkpoint 的正式顺序。所有步骤都通过 `run_v8.sh` 进入；1B 与 5B 只替换 model/runtime profile 和 window/runtime 输出目录。

## 1. 环境

```bash
export ROOT=/cluster/project/wm3d_v8
export WORK=/cluster/datasets/wm3d_v8
export RAW=$WORK/raw
...
```bash
./run_v8.sh collection-inventory \
  --data-template configs/data/public_robot_6106h.template.yaml \
