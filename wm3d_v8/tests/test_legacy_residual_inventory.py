...
from wm3d_v3.data.legacy_residual_inventory import (
...
from wm3d_v3.data.source_inventory import deterministic_split, validate_written_inventory
...
def _legacy_row(root: Path) -> dict[str, object]:
    return {
        ...
        # coordinates form one non-overlapping cover of the audited V8 arm7.
        "action_columns": [
            {
                "group_name": "arm_pose",
                "column": "action",
...
def test_real_parquet_video_import_rebuilds_v8_evidence(legacy_fixture, tmp_path: Path):
...
def test_v8_adapter_owns_legacy_camera_rename(legacy_fixture, tmp_path: Path):
...
def _template(path: Path) -> None:
    ...
    value = {
        ...
        "sources": [
            {"name": LEGACY_FORMAL_SOURCE, "embodiment": "legacy_arm_v8"}
        ],
        "embodiments": [
            {
                "name": "legacy_arm_v8",
