...
def test_step_receipt_is_no_clobber_and_detects_output_tamper(
    ...
    repo = tmp_path / "repo"
    repo.mkdir()
    run_v8 = _write(repo / "run_v8.sh", b"#!/bin/sh\nexit 0\n")
    run_v8.chmod(0o755)
    work = tmp_path / "work"
...
def test_run_v8_uses_distributed_preflight_and_has_no_transition() -> None:
    repo = Path(__file__).resolve().parents[1]
    text = (repo / "run_v8.sh").read_text(encoding="utf-8")
    assert "preflight) torchrun_preflight_split" in text
    assert '"${app_args[@]}" --preflight-only' in text
    assert "transition)" not in text
    assert "audit_wm3d_v8_stage0_libero_transition.py" not in text
    subprocess.run(["bash", "-n", str(repo / "run_v8.sh")], check=True)
