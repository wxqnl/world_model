...
def test_canary_launcher_defaults_to_fresh_v3_outputs() -> None:
    launcher = (
        ROOT / "scripts" /
        "launch_wm3d_v8_stage0_causal_dual_view_canary.sh"
    ).read_text()

    assert "runtime_config_smoke20_v3.yaml" in launcher
    assert "seal_report_smoke20_v3.json" in launcher
    assert "results/training_canary20_v3" in launcher
    assert "logs/training_canary20_v3" in launcher
