...
本文件只编排 ``run_v8.sh`` 已有的统一子命令，不实现第二套数据、训练或评测
...
class Orchestrator:
    def __init__(
        ...
        *,
        ...
        self.run_v8 = str(repo / "run_v8.sh")
    ...
    def step(
        ...
        *,
        ...
        command = [self.run_v8, *map(str, subcommand)]
        definition = {
...
def main(arguments: Sequence[str] | None = None) -> None:
    ...
    lineage = f"wm3d_v8_real_aloha_bimanual_smoke_{plan_sha[:12]}"
    ...
    if worker_receipt.exists():
        ...
        definition_command = [flow.run_v8, "cache-worker-pair", "receipt-only"]
        definition = {
