...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="SPLIT=ROOT_SHA256; repeat for train/val/test",
    )
    ...
    for raw in args.selection:
        ...
        if not separator or split not in {"train", "val", "test"} or split in selections:
            raise RuntimeError(f"invalid/duplicate selection {raw!r}")
        selections[split] = root_id
    if set(selections) != {"train", "val", "test"}:
        raise RuntimeError("selection must contain train/val/test exactly once")
    available = _rows(runtime_root)
    audited = []
    for split in ("train", "val", "test"):
        root_id = selections[split]
        ...
        if row.get("split") != split or Path(row["source_dataset"]).resolve(strict=True) != source_root:
        ...
        audited.append({
            "split": split, "root_id": root_id, "episode_id": episode, "t0": t0,
