"""Strict metadata-only import of audited V7 residual episode plans into V8.
...
are rebuilt under the V8 adapter/data-profile contracts.  No V7 training or
...
class LegacyResidualImportError(SourceInventoryError):
    """Raised when legacy metadata cannot satisfy the V8 source contract."""
...
def _validate_old_action_contract(
    ...
    *,
        ...
        for term in mapping.action:
            for coordinate in ((term.key, item) for item in term.columns):
                if coordinate in adapter_coordinates:
                    raise LegacyResidualImportError(
                        f"{identity}: V8 adapter reuses action coordinate {coordinate}"
                    )
                adapter_coordinates.add(coordinate)
    # Legacy V7 commonly split arm6 and gripper1 while V8 intentionally owns
    ...
    # solely responsible for grouping/semantics in V8.
    if legacy_coordinates != adapter_coordinates:
        ...
        raise LegacyResidualImportError(
            f"{identity}: legacy/V8 action coordinate coverage differs: "
    ...
    *,
    ...
    for index, raw in enumerate(value):
        ...
        if canonical_name is None or canonical_name not in view_slots:
            raise LegacyResidualImportError(
                f"{identity}/{name}: feature key is not mapped into the V8 view vocabulary"
            )
        if canonical_name in present:
            raise LegacyResidualImportError(
                f"{identity}: multiple legacy views map to V8 view {canonical_name!r}"
            )
    ...
    if observed_views != expected_views:
        raise LegacyResidualImportError(
            f"{identity}: present legacy views do not exactly match V8 adapter views"
        )
    if not present:
...
def import_legacy_residual_plan(
    *,
    ...
    """Rebuild a standard V8 manifest from one audited V7 residual plan."""
    ...
    if not view_slots or len(view_slots) != len(set(view_slots)):
        raise LegacyResidualImportError("V8 view_slots must be unique/non-empty")
    ...
    for line_number, legacy in enumerate(plan_rows, 1):
        ...
        if legacy["embodiment"] != embodiment.name:
            raise LegacyResidualImportError(f"{identity}: embodiment does not match V8 template")
        ...
        if str(legacy["timestamp_column"]) != adapter.observation_time_key:
            raise LegacyResidualImportError(
                f"{identity}: legacy timestamp column differs from V8 adapter"
            )
