...
class SourceHomogeneousDistributedBatchSampler(Sampler[list[int]]):
    ...
    ranks. ``set_start_batch`` deterministically advances both the shuffled
    source schedule and every per-source permutation stream without yielding
    the already-consumed batches, so an exact resume does not fetch or decode
    historical dataset samples.
    """
    ...
    def __init__(
        ...
        *,
        ...
        allow_small_source_replacement: bool = False,
    ) -> None:
        ...
        self.allow_small_source_replacement = bool(
            allow_small_source_replacement
        )
        self.epoch = 0
        self.start_batch = 0
        ...
        for name, (start, stop) in dataset.source_spans.items():
            if (
                ...
                and not self.allow_small_source_replacement
            ):
                raise ValueError(
    ...
    def set_start_batch(self, start_batch: int) -> None:
        ...
        if start < 0 or start > self.num_batches:
            raise ValueError(
                f"start_batch must be in [0, {self.num_batches}], got {start}"
            )
        self.start_batch = start
