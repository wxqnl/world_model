...
from .canonical_action import (
    ...
    CanonicalActionContractError,
    SUPPORTED_CANONICAL_ACTION_SOURCES,
    canonical_action_source,
    load_canonical_action_cache_manifest,
    load_canonical_action_stats,
    validate_canonical_action_metadata,
)
...
class WindowConfig:
    ...
    # Strict v7 action contract.  This path is opt-in so V6/V7 evaluation and
    # old checkpoints keep their exact legacy sample semantics.  Enabling it
    # requires an explicit allowlist and one source-bound v7 stats NPZ per
    # enabled source; legacy pooled action_stats are rejected.
...
def _action_history_before_index(
    actions,
    end_exclusive: int,
    hist_len: int,
    action_dim: int,
...
def _action_history_from_actions(actions, start: int, T: int, hist_len: int, action_dim: int):
    return _action_history_before_index(
        actions,
        start + T,
        hist_len,
        action_dim,
...
class OXEWindowDataset(Dataset):
    ...
    def __init__(self, records: list[OXEClipRecord], cfg: WindowConfig | None = None):
        ...
        self._canonical_action_sources: frozenset[str] = frozenset()
        self._canonical_action_stats = {}
        self._canonical_action_cache_manifest = None
        self._canonical_action_cache_records = {}
        self._canonical_action_array_cache: OrderedDict[
            str, tuple[np.ndarray, np.ndarray]
        ] = OrderedDict()
        self._canonical_action_array_cache_capacity = 128
        if self.cfg.canonical_action_enabled:
            if self.cfg.action_stats is not None:
                raise CanonicalActionContractError(
                    "canonical_action_enabled forbids legacy pooled action_stats; "
                    "use canonical_action_stats_by_source"
                )
            configured_sources = tuple(self.cfg.canonical_action_sources or ())
            if not configured_sources:
                raise CanonicalActionContractError(
                    "canonical_action_enabled requires a non-empty explicit "
                    "canonical_action_sources allowlist"
                )
            normalized_sources = tuple(
                canonical_action_source(source) for source in configured_sources
            )
            if len(set(normalized_sources)) != len(normalized_sources):
                raise CanonicalActionContractError(
                    "canonical_action_sources contains duplicate source keys"
                )
            unsupported = sorted(
                set(normalized_sources) - SUPPORTED_CANONICAL_ACTION_SOURCES
            )
            if unsupported:
                raise CanonicalActionContractError(
                    "action-enabled source has no passed canonical adapter: "
                    + ", ".join(unsupported)
                )
            self._canonical_action_sources = frozenset(normalized_sources)
            if len(self._canonical_action_sources) != 1:
                raise CanonicalActionContractError(
                    "one OXEWindowDataset must contain exactly one canonical "
                    "source/temporal offset; use separate Bridge and DROID loaders"
                )
            only_source = next(iter(self._canonical_action_sources))
            required_offset = CANONICAL_ACTION_FRAME_OFFSETS[only_source]
            if int(self.cfg.default_action_frame_offset) != required_offset:
                raise CanonicalActionContractError(
                    f"canonical source {only_source} requires explicit "
                    f"default_action_frame_offset={required_offset}, got "
                    f"{self.cfg.default_action_frame_offset}"
                )
            raw_stats = self.cfg.canonical_action_stats_by_source
            if not isinstance(raw_stats, dict):
                raise CanonicalActionContractError(
                    "canonical_action_stats_by_source must map each enabled "
                    "source to a source-bound v7 NPZ"
                )
            normalized_stats_paths: dict[str, Path] = {}
            for raw_source, raw_path in raw_stats.items():
                source = canonical_action_source(raw_source)
                if source in normalized_stats_paths:
                    raise CanonicalActionContractError(
                        f"duplicate canonical action stats source {source!r}"
                    )
                normalized_stats_paths[source] = Path(raw_path)
            missing_stats = sorted(
                self._canonical_action_sources - set(normalized_stats_paths)
            )
            extra_stats = sorted(
                set(normalized_stats_paths) - self._canonical_action_sources
            )
            ...
            configured_source_manifest_sha256 = str(
                self.cfg.trusted_manifest_sha256 or ""
            )
            for source, stats in self._canonical_action_stats.items():
                if (
                    stats.source_manifest_sha256
                    != configured_source_manifest_sha256
                ):
                    raise CanonicalActionContractError(
                        f"canonical stats for {source} are not bound to "
                        "trusted_manifest_sha256"
                    )
            if (
                self.cfg.canonical_action_cache_manifest is None
                or not self.cfg.canonical_action_cache_manifest_sha256
            ):
                raise CanonicalActionContractError(
                    "canonical_action_enabled requires a pinned "
                    "canonical_action_cache_manifest and SHA256"
                )
            self.cfg.canonical_action_cache_manifest = Path(
                self.cfg.canonical_action_cache_manifest
            )
            self._canonical_action_cache_manifest = (
                load_canonical_action_cache_manifest(
                    self.cfg.canonical_action_cache_manifest,
                    expected_sha256=self.cfg.canonical_action_cache_manifest_sha256,
                    expected_sources=self._canonical_action_sources,
                )
            )
            self._canonical_action_cache_records = dict(
                self._canonical_action_cache_manifest.entries
            )
            for source, stats in self._canonical_action_stats.items():
                if (
                    stats.action_cache_manifest_sha256
                    != self._canonical_action_cache_manifest.sha256
                ):
                    raise CanonicalActionContractError(
                        f"canonical stats for {source} are not bound to the "
                        "runtime canonical action cache manifest"
                    )
            for record in records:
                source = canonical_action_source(record.dataset)
                if source in self._canonical_action_sources:
                    validate_canonical_action_metadata(
                        source=source,
                        action_kind=record.action_kind,
                    )
                    entry = self._canonical_action_cache_records.get(record.clip_id)
                    if entry is None:
                        raise CanonicalActionContractError(
                            "canonical action cache manifest omits enabled clip "
                            f"{record.clip_id}"
                        )
                    if (
                        entry.source != source
                        or entry.action_kind != record.action_kind
                        or entry.n_frames != int(record.n_frames)
                        or entry.action_frame_offset
                        != CANONICAL_ACTION_FRAME_OFFSETS[source]
                    ):
                        raise CanonicalActionContractError(
                            "canonical action cache metadata disagrees with source "
                            f"manifest for {record.clip_id}"
                        )
                    expected_action_count = int(record.n_frames) - (
                        1 if source == "droid" else 0
                    )
                    if entry.action_count != expected_action_count:
                        raise CanonicalActionContractError(
                            f"canonical action count for {record.clip_id} is "
                            f"{entry.action_count}, expected {expected_action_count}"
                        )
            for source in self._canonical_action_sources:
                source_evidence = {
                    entry.action_contract_evidence_sha256
                    for entry in self._canonical_action_cache_records.values()
                    if entry.source == source
                }
                if len(source_evidence) != 1:
                    raise CanonicalActionContractError(
                        f"canonical source {source} must bind exactly one temporal "
                        "action-contract evidence digest"
                    )
                configured_evidence = str(
                    self.cfg.action_contract_evidence_sha256 or ""
                )
                if source_evidence != {configured_evidence}:
                    raise CanonicalActionContractError(
        ...
        self._all_records_canonical = bool(
            ...
            and all(
                canonical_action_source(record.dataset)
                in self._canonical_action_sources
                for record in records
            )
        )
        # A promoted canonical cache already pins the raw DROID index and the
        # formal temporal evidence through its generator/gate.  Re-reading and
        # decoding the 98 MiB raw index plus every state sidecar independently
        # in each DDP rank is both redundant and prohibitively slow.  Preserve
        # the legacy path only for datasets that are not wholly canonical.
        self._legacy_runtime_action_contract = bool(
            self.cfg.require_action_contract and not self._all_records_canonical
        )
        has_droid = any(
        ...
        if self._legacy_runtime_action_contract and has_droid:
        ...
        if self.cfg.action_contract_path is not None and self._legacy_runtime_action_contract:
        ...
        if self._legacy_runtime_action_contract:
        ...
        trusted_manifest_shard_fast_init = (
            ...
            and not self.cfg.load_policy_state
            and not self.cfg.require_progress
            and not self._legacy_runtime_action_contract
            and self._window_shards is not None
        )
        for r in records:
            cid = _safe(r.clip_id)
            record_frame_count = int(getattr(r, "n_frames", 0) or 0)
            canonical_source = canonical_action_source(r.dataset)
            canonical_entry = (
                self._canonical_action_cache_records.get(r.clip_id)
                if canonical_source in self._canonical_action_sources
                else None
            )
            if trusted_manifest_shard_fast_init:
                ...
                starts = self._filter_action_starts(
                    ...
                    starts,
                    (
                        canonical_entry.action_count
                        if canonical_entry is not None
                        else record_frame_count
                    ),
                )
                if not starts:
                    continue
                ...
                continue
            if canonical_entry is not None:
                action_len = canonical_entry.action_count
                action_valid_count = action_len
            else:
                action_cache = resolve_action_cache(
                    r,
                    cache_root=self.cfg.cache_root,
                    formal_stage1=self._legacy_runtime_action_contract,
                )
    ...
    def _action_offset_for(self, record: OXEClipRecord) -> int:
        source = canonical_action_source(record.dataset)
        canonical_sources = getattr(self, "_canonical_action_sources", frozenset())
        if source in canonical_sources:
            entry = getattr(self, "_canonical_action_cache_records", {}).get(
                record.clip_id
            )
            if entry is None:
                raise CanonicalActionContractError(
                    f"canonical action cache omits {record.clip_id}"
                )
            return int(entry.action_frame_offset)
        legacy_runtime_contract = getattr(
            self,
            "_legacy_runtime_action_contract",
            bool(self.cfg.require_action_contract),
        )
        if not legacy_runtime_contract and not self._action_frame_offsets:
    ...
    def _load_canonical_action_episode(
        self, clip_id: str
    ) -> tuple[np.ndarray, np.ndarray]:
        cached = self._canonical_action_array_cache.get(clip_id)
        if cached is not None:
            self._canonical_action_array_cache.move_to_end(clip_id)
            return cached
        entry = self._canonical_action_cache_records.get(clip_id)
        if entry is None:
            raise CanonicalActionContractError(
                f"canonical action cache manifest omits {clip_id}"
            )
        arrays = entry.load_arrays()
        self._canonical_action_array_cache[clip_id] = arrays
        self._canonical_action_array_cache.move_to_end(clip_id)
        while (
            len(self._canonical_action_array_cache)
            > self._canonical_action_array_cache_capacity
        ):
            self._canonical_action_array_cache.popitem(last=False)
        return arrays
    ...
    def __getitem__(self, i: int) -> dict:
        ...
        canonical_source = canonical_action_source(rec.dataset)
        canonical_action_valid = False
        canonical_action_valid_mask = np.zeros(7, dtype=np.bool_)
        canonical_stats = None
        if canonical_source in self._canonical_action_sources:
            episode_actions, episode_grip_close01 = (
                self._load_canonical_action_episode(rec.clip_id)
            )
            actions = episode_actions
            action_valid_count = int(episode_actions.shape[0])
            action_cache_subdir = "canonical_action_v1"
            canonical_action_valid_mask = np.ones(7, dtype=np.bool_)
            canonical_action_valid = True
            canonical_stats = self._canonical_action_stats[canonical_source]
        else:
            action_cache = resolve_action_cache(
                rec,
                cache_root=self.cfg.cache_root,
            ...
            if self.cfg.canonical_action_enabled:
                # Representation-only sources must not leak unaudited controls
                # into factual dynamics or policy history.  The explicit mask
                # is still emitted so downstream code can exclude their action
                # losses while retaining their RGB/geometry supervision.
                episode_actions = np.zeros(
                    (int(actions.shape[0]), 7), dtype=np.float32
                )
                episode_grip_close01 = np.zeros(
                    int(actions.shape[0]), dtype=np.float32
                )
            else:
                episode_actions = actions
                episode_grip_close01 = np.asarray(actions[:, 6], dtype=np.float32)
        ...
        action_indices = list(action_resolution.action_frame_indices)
        act_w = np.array(episode_actions[action_indices], dtype=np.float32)
        prev_grip_signed = np.array(
            [episode_actions[action_resolution.previous_gripper_index, 6]],
            dtype=np.float32,
        )
        if self.cfg.canonical_action_enabled:
            grip_close01_w = np.array(
                episode_grip_close01[action_indices], dtype=np.float32
            )
            # Keep the long-standing prev-grip input in close01 space.  The
            # physical signed value is emitted separately and never fed to BCE
            # as a target.
            prev_grip = np.array(
                [episode_grip_close01[action_resolution.previous_gripper_index]],
                dtype=np.float32,
            )
        else:
            grip_close01_w = np.asarray(act_w[:, 6], dtype=np.float32)
            prev_grip = prev_grip_signed
        ...
        if self.cfg.canonical_action_enabled:
            if canonical_stats is not None:
                action_tgt_norm = canonical_stats.normalize_pose(act_w[:, :6])
                action_pose_mean = np.asarray(canonical_stats.mean, dtype=np.float32)
                action_pose_std = np.asarray(canonical_stats.std, dtype=np.float32)
            else:
                action_tgt_norm = np.zeros((len(act_w), 6), dtype=np.float32)
                action_pose_mean = np.zeros(6, dtype=np.float32)
                action_pose_std = np.ones(6, dtype=np.float32)
        elif self.act_mean is not None:
            action_tgt_norm = (act_w[:, :6] - self.act_mean) / self.act_std
            action_pose_mean = np.asarray(self.act_mean, dtype=np.float32)
            action_pose_std = np.asarray(self.act_std, dtype=np.float32)
        else:
        ...
        sample = {
            ...
            "action_pose_mean": torch.from_numpy(action_pose_mean.copy()).float(),
            "action_pose_std": torch.from_numpy(action_pose_std.copy()).float(),
            "action_prev_grip": torch.from_numpy(prev_grip).float(),
            "c":               torch.from_numpy(np.asarray(qwen, dtype=np.float16)).float(),
            "clip_id":         rec.clip_id,
            "start":           start,
            "dataset":         rec.dataset,
            "action_cache_subdir": action_cache_subdir,
        ...
        if self.cfg.canonical_action_enabled:
            sample.update(
                {
                    # action_tgt is the physical canonical vector and therefore
                    # carries a signed close channel.  BCE users must consume
                    # action_grip_close01 instead.
                    "action_grip_close01": torch.from_numpy(
                        grip_close01_w
                    ).float(),
                    "action_prev_grip_signed": torch.from_numpy(
                        prev_grip_signed
                    ).float(),
                    "action_valid": torch.tensor(
                        canonical_action_valid, dtype=torch.bool
                    ),
                    "action_valid_mask": torch.from_numpy(
                        canonical_action_valid_mask.copy()
                    ),
                    "action_source": canonical_source,
                    "action_stats_key": (
                        canonical_stats.stats_key
                        if canonical_stats is not None
                        else f"invalid:{canonical_source}"
                    ),
                    "action_contract_version": CANONICAL_ACTION_SCHEMA,
                    "action_signal_role": "commanded_delta_not_observed_ee_delta",
                    "action_translation_frame": "robot_base",
                    "action_rotation_convention": "so3_log_base_from_fixed_xyz_rz_ry_rx",
                    "action_grip_representation": "signed_close_physical+close01_supervision",
                }
            )
        ...
        if (
            ...
            if self.cfg.canonical_action_enabled:
                ...
                # masquerading as rotvec, or an ambiguous grip convention.
                history_close01_actions = np.array(
                    episode_actions, dtype=np.float32, copy=True
                )
                history_close01_actions[:, 6] = episode_grip_close01
                hist = _action_history_before_index(
                    history_close01_actions,
                    action_indices[0],
                    self.cfg.policy_action_history_len,
                    self.cfg.policy_action_history_dim,
                )
                hist_signed = _action_history_before_index(
                    episode_actions,
                    action_indices[0],
                    self.cfg.policy_action_history_len,
                    self.cfg.policy_action_history_dim,
                )
                sample["action_history_signed"] = torch.from_numpy(
                    hist_signed
                ).float()
            else:
                hist_arr = (
                    _npz_first(geom, ("action_history", "action_hist"))
                    if geom is not None
                    else None
                )
                if hist_arr is None and self.cfg.load_policy_state:
                    hist_arr = _load_cache_array(
                        self.cfg.cache_root,
                        "action_history",
                        cid,
                    )
                if hist_arr is not None:
                    hist_arr = _select_frame_or_global(hist_arr, start + T - 1)
