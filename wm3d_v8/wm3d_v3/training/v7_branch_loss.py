...
def true_branch_reconstruction_matching_loss(
    ...
    *,
    ...
    effect_norm_loss = F.smooth_l1_loss(
        torch.log1p(predicted_effect_flat.norm(dim=-1)),
        torch.log1p(target_effect_flat.norm(dim=-1)),
        beta=0.1,
    )
    # Positive means the predicted effect beats the collapsed zero-effect
    # baseline under exactly the same branch/effect mask.
    effect_model_error = effect_error_mse.mean()
    effect_zero_error = target_effect_mse.mean()
    effect_gain_vs_zero = 1.0 - effect_model_error / effect_zero_error.clamp_min(
        settings.effect_min_rms**2
    )
    ...
    loss = (
        ...
        + settings.effect_norm_weight * effect_norm_loss
    )
    return {
        "loss": loss,
        ...
        "effect_norm_loss": effect_norm_loss,
        "effect_gain_vs_zero": effect_gain_vs_zero,
        "effect_model_error": effect_model_error,
        "effect_zero_error": effect_zero_error,
