...
class JointWorldModel(nn.Module):
    ...
    def propose_imagine_select(
        ...
        *,
        ...
        include_direct_candidate: bool = True,
        preserve_direct_gripper: bool = False,
        proposal_flow_blend: float = 1.0,
        proposal_pose_clip: float | None = None,
        include_geometry: bool = False,
    ) -> dict[str, torch.Tensor]:
        """Execute the native flow-propose -> WM-imagine -> value-select loop.

        The proposal context is produced by one action-free dual pass.  K
        independent flow samples are then rolled through the action-conditioned
        native world model and scored by the task-grounded future-value head.
        """

        if self.action_policy is None or self.future_value_head is None:
            raise RuntimeError(
                "propose_imagine_select requires action_policy and future_value_head"
            )
        policy_source = str(self.cfg.policy_context_source).strip().lower()
        attached_sources = {
            "core",
            "core_pred",
            "pred",
        ...
        if policy_source not in attached_sources | detached_sources:
            raise RuntimeError(
                "native planning requires a core-prediction policy context"
            )
        policy_action_mode = str(self.cfg.policy_core_action_cond).strip().lower()
        if policy_action_mode not in {"none", "no_action", "off", "disabled"}:
            raise RuntimeError(
                "native planning proposals must use an action-free core prediction"
            )
        candidates = int(num_candidates)
        if candidates < 2:
            raise ValueError("native planning requires at least two proposals")
        if s.ndim != 4 or c.ndim != 2 or int(s.shape[0]) != int(c.shape[0]):
            raise ValueError("expected s[B,T,P,D] and c[B,D]")

        s = self.decode_input_tokens(s)
        bsz = int(s.shape[0])
        no_action = self.dual(s, c, action_cond=None)
        policy_tokens = no_action["pred_tokens"]
        if policy_source in detached_sources:
            policy_tokens = policy_tokens.detach()
        expanded_tokens = policy_tokens[:, None].expand(
            -1, candidates, -1, -1, -1
        ).reshape(bsz * candidates, *policy_tokens.shape[1:])
        expanded_task = c[:, None].expand(-1, candidates, -1).reshape(
            bsz * candidates, -1
        )

        def expand_optional(value: torch.Tensor | None) -> torch.Tensor | None:
            if value is None:
                return None
            if int(value.shape[0]) != bsz:
                raise ValueError(
                    "native planning auxiliary inputs must share the state batch"
                )
            return value[:, None].expand(
                -1, candidates, *([-1] * (value.ndim - 1))
            ).reshape(bsz * candidates, *value.shape[1:])
        ...
        policy = self.action_policy(
            ...
            progress_state=expand_optional(progress_state),
            context_rgb=expand_optional(context_rgb),
            flow_sample=True,
            flow_sample_steps=flow_sample_steps,
            flow_noise_scale=flow_noise_scale,
        )
        if "policy_flow_action_cond" not in policy:
            raise RuntimeError(
                "native planning requires policy_enable_flow_head=true"
            )
        flow_actions = policy["policy_flow_action_cond"].reshape(
            bsz,
            candidates,
            int(policy["policy_flow_action_cond"].shape[1]),
            int(policy["policy_flow_action_cond"].shape[2]),
        )
        direct_actions = policy["policy_action_cond"].reshape(
            bsz,
            candidates,
            int(policy["policy_action_cond"].shape[1]),
            int(policy["policy_action_cond"].shape[2]),
        )
        proposal_actions = flow_actions.clone()
        blend = float(proposal_flow_blend)
        if not 0.0 < blend <= 1.0:
            raise ValueError("proposal_flow_blend must be in (0, 1]")
        if blend < 1.0:
            proposal_actions[..., :6] = (
                direct_actions[..., :6]
                + blend
        ...
        if proposal_pose_clip is not None:
            clip = float(proposal_pose_clip)
            if clip <= 0:
                raise ValueError("proposal_pose_clip must be positive")
            proposal_actions[..., :6] = proposal_actions[..., :6].clamp(
                min=-clip,
                max=clip,
            )
        if preserve_direct_gripper:
            proposal_actions[..., 6] = direct_actions[..., 6]
        if include_direct_candidate:
            proposal_actions[:, 0] = direct_actions[:, 0]
        imagined = self.imagine_candidates(
            s,
            c,
            proposal_actions,
            include_geometry=include_geometry,
        )
        values = self.future_value_head(imagined["pred_tokens"], c)
        logits = values["candidate_success_logit"]
        selected_index = logits.argmax(dim=1)
        action_gather = selected_index[:, None, None, None].expand(
            -1,
            1,
            int(proposal_actions.shape[2]),
            int(proposal_actions.shape[3]),
        )
        future_gather = selected_index[:, None, None, None, None].expand(
            -1,
            1,
            *imagined["pred_tokens"].shape[2:],
        )
        return {
            "proposal_actions": proposal_actions,
            "proposal_pose_norm": proposal_actions[..., :6],
            "proposal_gripper_probability": proposal_actions[..., 6],
            "direct_action": direct_actions[:, 0],
            "flow_proposal_actions": flow_actions,
            "candidate_pred_tokens": imagined["pred_tokens"],
            "candidate_progress_logit": values["candidate_progress_logit"],
            "candidate_success_logit": logits,
            "selected_index": selected_index,
            "selected_action": proposal_actions.gather(1, action_gather).squeeze(1),
            "selected_pred_tokens": imagined["pred_tokens"]
            .gather(1, future_gather)
            .squeeze(1),
            "action_free_pred_tokens": policy_tokens,
        }
    ...
    def forward(self, s: torch.Tensor, c: torch.Tensor,
        ...
        # S1 can need one action-free dual pass for both native inverse action
        # supervision and the flow policy's future-token context. Cache it so
        # enabling both heads does not turn two dual passes into three.
        no_teacher_dual: dict[str, torch.Tensor] | None = None
            ...
            out.update(
                {
                    ...
                    "native_action_no_teacher_pred_tokens": no_teacher_dual[
                        "pred_tokens"
                    ],
                }
            )
            ...
            if policy_source in {"", "input", "s", "tokens", "cached"}:
                ...
                if policy_core_action_mode in {"", "same", "teacher", "gt", "action", "action_cond"}:
                    ...
                    if core_action_cond is None:
                        ...
                        if no_teacher_dual is None:
                            no_teacher_dual = self.dual(s, c, action_cond=None)
                        policy_tokens = no_teacher_dual["pred_tokens"]
                else:
                ...
                if policy_core_action_mode in {"", "same", "teacher", "gt", "action", "action_cond"}:
                    ...
                    if core_action_cond is None:
                        ...
                        if no_teacher_dual is None:
                            with torch.no_grad():
                                no_teacher_dual = self.dual(s, c, action_cond=None)
                        policy_tokens = no_teacher_dual["pred_tokens"].detach()
                else:
                    raise ValueError(f"unsupported policy_core_action_cond={policy_core_action_mode!r}")
            else:
