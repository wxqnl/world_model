...
def _forward_world(
    ...
    *,
    ...
    pixel: bool,
    policy_kwargs: dict[str, torch.Tensor] | None = None,
    skip_policy_heads: bool = False,
    ...
    if wrist_tokens is not None:
        kwargs["wrist_s"] = wrist_tokens
        kwargs["view_mask"] = view_mask
    if policy_kwargs:
        kwargs.update(policy_kwargs)
    if skip_policy_heads and hasattr(_unwrap_model(model), "action_policy"):
        kwargs.update(
...
def _policy_residual_candidates(
    first_out: dict[str, torch.Tensor],
    *,
    scales: tuple[float, ...] = (0.25, 0.5),
) -> torch.Tensor:
    """Build anchor plus scaled proposer residuals used by the K9 contract."""
    if "policy_action_cond" not in first_out:
        raise RuntimeError(
            "selection_mode=ranked_residual requires policy_action_cond"
        )
    if "proposer_action_cond" not in first_out:
        raise RuntimeError(
            "selection_mode=ranked_residual requires proposer_action_cond"
        )
    anchor = first_out["policy_action_cond"].float()
    proposals = first_out["proposer_action_cond"].float()
    if anchor.ndim != 3 or anchor.shape[-1] != 7:
        raise ValueError(f"expected policy_action_cond [B,k,7], got {tuple(anchor.shape)}")
    if proposals.ndim != 4 or proposals.shape[-1] != 7:
        raise ValueError(
            f"expected proposer_action_cond [B,K,k,7], got {tuple(proposals.shape)}"
        )
    if proposals.shape[0] != anchor.shape[0] or proposals.shape[2:] != anchor.shape[1:]:
        raise ValueError(
            "policy/proposer candidate shape mismatch: "
            f"anchor={tuple(anchor.shape)} proposals={tuple(proposals.shape)}"
        )
    residuals = proposals - anchor[:, None]
    candidates = [anchor[:, None]]
    candidates.extend(anchor[:, None] + float(scale) * residuals for scale in scales)
...
def select_action_chunk(
    ...
    *,
    ...
    if selection_mode in (
        ...
        policy_model = anchor_model if anchor_model is not None else model
        target = _unwrap_model(policy_model)
        ...
        selected_raw = denormalize_action_cond(selected_cond, model=policy_model)
    ...
    if selection_mode == "ranked_residual" and anchor_model is not None:
        anchor_target = _unwrap_model(anchor_model)
        if getattr(anchor_target, "action_policy", None) is None:
            raise RuntimeError(
                "selection_mode=ranked_residual anchor_model requires enable_action_policy"
            )
        anchor_out = anchor_target.act_policy(
            s,
            task_emb,
            lowdim_state=lowdim_state,
            object_state=object_state,
        ...
        if "policy_action_cond" not in anchor_out:
            raise RuntimeError("anchor_model did not emit policy_action_cond")
        first_out = dict(first_out)
        first_out["policy_action_cond"] = anchor_out["policy_action_cond"].float()
    if "proposer_action_cond" not in first_out:
        raise RuntimeError("model output has no proposer_action_cond; enable_action_proposer is required")

    candidate_cond = (
        _policy_residual_candidates(first_out)
        if selection_mode == "ranked_residual"
        else first_out["proposer_action_cond"].float()
    )
    ...
    if selection_mode in ("ranked", "ranked_residual", "score", "evaluator"):
        ...
        raise ValueError(
            f"unknown selection_mode={selection_mode!r}; expected ranked, "
            "ranked_residual, anchor, direct, or plan_waypoint"
        )
    ...
    action_stats_model = anchor_model if anchor_model is not None else model
    selected_raw = denormalize_action_cond(selected_cond, model=action_stats_model)
