"""Step-addressed, exact-ratio sampler for native WM3D-V7 5B.

The sampler has no opaque iterator state.  Given the source spans, seed,
optimizer step, world size, rank, micro-batch size and gradient accumulation,
...
def _cycle_permutation(length: int, seed: int, cycle: int) -> tuple[int, ...]:
    """Stateless Fisher-Yates permutation for a usually small source cycle."""
...
class ExactSourceSchedule:
    """Exact integer-weight schedule with deterministic per-cycle shuffling."""
    ...
    def address(self, optimizer_step: int) -> StepAddress:
        ...
        source_id = cycle_sources[position]
        source_name = self.source_order[source_id]
        occurrences = cycle * self.weights[source_name]
        occurrences += sum(
            1 for earlier in cycle_sources[:position] if earlier == source_id
        )
        return StepAddress(
            optimizer_step=step,
...
class AffinePermutation:
    """O(1) deterministic permutation stream with no replacement per pass."""
...
class StepAddressedBatchSampler(Sampler[list[int]]):
    """Yield local micro-batches for a range of optimizer steps.

    Dataset ``source_spans`` must map each source to a half-open span in a
    concatenated mixed dataset.  All micro-batches belonging to one optimizer
    step use the same source, so source-specific contracts remain homogeneous
    ...
    def __init__(
        ...
        *,
        ...
        self.source_spans = {
            ...
            for name, span in source_spans.items()
        }
        ...
        if set(self.source_spans) != set(self.schedule.source_order):
            raise SamplingContractError(
                "source spans must exactly match the scheduled sources"
            )
        ...
        for source_name, (start, stop) in self.source_spans.items():
            ...
            if length < self.global_batch:
                raise SamplingContractError(
                    ...
                    f"{self.global_batch}; no-replacement step cannot be formed"
                )
    ...
    def __iter__(self) -> Iterator[list[int]]:
        for step in range(
            ...
            self.start_optimizer_step + self.num_optimizer_steps,
        ):
            address = self.schedule.address(step)
            source_name = address.source_name
            source_start, _source_stop = self.source_spans[source_name]
            permutation = self._permutations[source_name]
            step_base = address.source_occurrence * self.global_batch
            for micro_step in range(self.gradient_accumulation):
                micro_base = step_base + micro_step * self.global_micro_batch
                rank_base = micro_base + self.rank * self.micro_batch_size
                yield [
