"""Immutable shard loader for native WM3D-V7 5B windows.
...
from .scale5b_codec import JpegPackReader, dequantize_per_vector


WINDOW_INDEX_SCHEMA = "wm3d_v7_native5b_window_index_v1"
...
class Native5BSourceDataset(Dataset[dict[str, torch.Tensor]]):
    """One immutable source/split of canonical native5b windows."""
...
class Native5BMixedDataset(Dataset[dict[str, torch.Tensor]]):
    """Concatenate sources in the exact sealed source order."""

    def __init__(self, sources: Sequence[tuple[str, Native5BSourceDataset]]) -> None:
        if not sources:
            raise DataIntegrityError("mixed native5b dataset has no sources")
        self.source_names = tuple(str(name) for name, _dataset in sources)
        if len(set(self.source_names)) != len(self.source_names):
            raise DataIntegrityError("mixed native5b source names are not unique")
