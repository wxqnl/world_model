...
class WanTI2VControlConfig:
    ...
    parallel_action_video_delta_norm_cap_ratio: float = 0.0
...
class WanTI2VControlAdapter(nn.Module):
    ...
    def _rgb_to_bcthw(self, x: torch.Tensor | None, *, batch: int, latent_t: int, latent_h: int, latent_w: int, name: str) -> torch.Tensor | None:
        ...
        if x.ndim == 4:
            ...
            # A still context is constant along latent time. Resize it in 2D
            # before making the temporal broadcast view; materializing the
            # full-resolution temporal repeat can consume hundreds of MiB.
            ctx = F.interpolate(
                ctx,
            ...
            return ctx[:, :, None].expand(-1, -1, int(latent_t), -1, -1)
        if x.ndim == 5:
    ...
    def parallel_action_video_delta(self, tokens: torch.Tensor, layer_idx: int) -> torch.Tensor:
        ...
        cap_ratio = float(getattr(self.cfg, "parallel_action_video_delta_norm_cap_ratio", 0.0) or 0.0)
        if cap_ratio > 0.0:
            # Clamp before sqrt: sqrt(0) has an infinite backward gradient even
            # if the forward result is clamped afterwards.
            delta_rms = video_delta.float().square().mean(dim=-1, keepdim=True).clamp_min(1e-12).sqrt()
            token_rms = tokens.float().square().mean(dim=-1, keepdim=True).sqrt().detach().clamp_min(1e-6)
            cap = token_rms.to(device=video_delta.device, dtype=video_delta.dtype) * cap_ratio
            video_delta = video_delta * (cap / delta_rms.to(device=video_delta.device, dtype=video_delta.dtype)).clamp(max=1.0)
        return video_delta * scale

    def parallel_action_probe_delta(
        self,
        token_len: int,
        *,
        ...
        """Cheap training probe for the actual action-video injection path.

        This reuses the same parallel action blocks that are added inside Wan
        transformer layers, but runs them on a small adapter-derived token set.
        It intentionally avoids mutating ``control_state`` so it can compare
        true and counterfactual actions without a second full Wan forward.
        """
        ...
        if (
            ...
            or (state.video_action_tokens is None and state.action_tokens is None)
            or not bool(self.cfg.use_parallel_action_video_blocks)
            or len(self.parallel_action_blocks) <= 0
        ):
            return None
        video_tokens = state.video_action_tokens if state.video_action_tokens is not None else state.action_tokens
        if video_tokens is None:
        ...
        tokens = self.to_dit(self._sequence_features(token_len, latent_shape))
        tokens = tokens.to(device=video_tokens.device, dtype=video_tokens.dtype)
        max_tokens = int(max_tokens or 0)
        if max_tokens > 0 and int(tokens.shape[1]) > max_tokens:
            idx = torch.linspace(0, int(tokens.shape[1]) - 1, max_tokens, device=tokens.device).round().long().unique()
            tokens = tokens.index_select(1, idx)
        if layer_indices is None:
            layers = (len(self.parallel_action_blocks) - 1,)
        else:
            layers = tuple(int(v) for v in layer_indices)
        stream = video_tokens.to(device=tokens.device, dtype=tokens.dtype)
        outputs: list[torch.Tensor] = []
        for layer_idx in layers:
            if layer_idx < 0:
                layer_idx = len(self.parallel_action_blocks) + layer_idx
            if layer_idx < 0 or layer_idx >= len(self.parallel_action_blocks):
                continue
            stream, video_delta = self.parallel_action_blocks[layer_idx](stream, tokens)
            scale = float(state.scale) * float(self.cfg.parallel_action_video_scale) * self._layer_gain(layer_idx)
            if bool(self.cfg.use_vam_action_expert):
                scale *= float(self.cfg.vam_action_video_delta_scale)
            gate = state.video_action_gate_tokens
            if gate is not None:
                gate_seq = self._gate_to_token_sequence(gate, video_delta)
                if gate_seq is not None:
                    video_delta = video_delta * gate_seq.to(device=video_delta.device, dtype=video_delta.dtype)
            cap_ratio = float(getattr(self.cfg, "parallel_action_video_delta_norm_cap_ratio", 0.0) or 0.0)
            if cap_ratio > 0.0:
                delta_rms = video_delta.float().square().mean(dim=-1, keepdim=True).clamp_min(1e-12).sqrt()
                token_rms = tokens.float().square().mean(dim=-1, keepdim=True).sqrt().detach().clamp_min(1e-6)
                cap = token_rms.to(device=video_delta.device, dtype=video_delta.dtype) * cap_ratio
                video_delta = video_delta * (cap / delta_rms.to(device=video_delta.device, dtype=video_delta.dtype)).clamp(max=1.0)
            scaled = video_delta * scale
            outputs.append(scaled)
            tokens = tokens + scaled.to(device=tokens.device, dtype=tokens.dtype)
        if not outputs:
            return None
...
def load_wan_ti2v_control_checkpoint(path: str | Path, device: str | torch.device | None = None) -> tuple[WanTI2VControlAdapter, dict[str, Any]]:
    ...
    cfg_payload = payload.get("wan_control_adapter_cfg") or payload.get("adapter_cfg") or payload.get("cfg")
    cfg = _cfg_from_payload(cfg_payload, ckpt_path=ckpt_path)
