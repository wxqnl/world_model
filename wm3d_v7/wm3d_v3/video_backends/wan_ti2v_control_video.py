...
def _backend_startup_log(event: str, started: float, device: torch.device, *, status: str = "ok", cached: bool = False) -> None:
    if device.type == "cuda" and torch.cuda.is_available():
        scale = float(1024**2)
        allocated = float(torch.cuda.memory_allocated(device)) / scale
        reserved = float(torch.cuda.memory_reserved(device)) / scale
        peak = float(torch.cuda.max_memory_allocated(device)) / scale
    else:
    ...
    print(
        f"[startup] stage=wan_backend.load event={event} monotonic_s={now:.6f}{elapsed} "
        f"status={status} cached={cached} rank={rank} rss_mib={_rss_mib():.1f} "
        f"cuda_alloc_mib={allocated:.1f} cuda_reserved_mib={reserved:.1f} cuda_peak_mib={peak:.1f}",
        flush=True,
    )
...
class WanTI2VControlVideoBackend:
    ...
    def load(self):
        ...
        _backend_startup_log("begin", started, self.device, cached=self._pipeline is not None)
        if self._pipeline is not None:
            _backend_startup_log("end", started, self.device, cached=True)
        ...
        try:
            ...
            _backend_startup_log("end", started, self.device, status="error")
            raise
        self._pipeline = pipeline
        _backend_startup_log("end", started, self.device)
        return pipeline
