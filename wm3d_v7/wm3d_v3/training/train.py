...
def build_datasets(cfg: dict, overfit_ids=None):
    ...
    if data_cfg.get("dataset_type") == "v7_mixed":
        ...
        oxe_cfg = dict(data_cfg.get("oxe") or {})
        if not oxe_cfg:
            raise ValueError("data.oxe is required for dataset_type=v7_mixed")
        for key in ("T", "k"):
        ...
        include_datasets = {
            str(name) for name in oxe_cfg.get("include_datasets", ())
        }
        if not include_datasets:
            raise ValueError(
                "mixed OXE input requires an explicit include_datasets compatibility cohort"
            )
        records = [
            record for record in records if str(record.dataset) in include_datasets
        ]
        if not records:
            raise RuntimeError(
                f"OXE compatibility cohort is empty: {sorted(include_datasets)}"
            )
        allowed_action_kinds = {
            str(kind) for kind in oxe_cfg.get("allowed_action_kinds", ())
        }
        observed_action_kinds = {str(record.action_kind) for record in records}
        if allowed_action_kinds and not observed_action_kinds.issubset(
            allowed_action_kinds
        ):
            raise ValueError(
                "OXE compatibility cohort contains unapproved action kinds: "
                f"{sorted(observed_action_kinds - allowed_action_kinds)}"
            )
        allowed_fps = {
            float(value) for value in oxe_cfg.get("allowed_fps", ())
        }
        observed_fps = {float(record.fps) for record in records}
        if allowed_fps and not observed_fps.issubset(allowed_fps):
            raise ValueError(
                "OXE compatibility cohort contains unapproved frame rates: "
        ...
        if evidence_path:
            ...
            for contract_key in sorted(
                ...
                if (
                    ...
                    raise ValueError(
                        "OXE compatibility cohort lacks matching passed temporal "
                        f"evidence for {contract_key}: expected_offset="
                        f"{configured_offset}"
                    )
        split_cfg = _data_split_cfg(oxe_cfg)
        split_mode = split_cfg.get("mode", "episode")
        if split_mode != "episode":
            raise ValueError("mixed OXE input requires an episode-level split")
        oxe_split = episode_split(
            records,
        ...
        oxe_train_records = [
            ...
            if record.clip_id in oxe_split.train_clip_ids
        ]
        oxe_val_records = [
            record for record in records if record.clip_id in oxe_split.val_clip_ids
        ]
        oxe_window_cfg = _window_config(oxe_cfg, cfg.get("model"))
        oxe_train = OXEWindowDataset(oxe_train_records, oxe_window_cfg)
        oxe_val = OXEWindowDataset(oxe_val_records, oxe_window_cfg)
        if len(oxe_train) == 0 or len(oxe_val) == 0:
            raise RuntimeError(
                "mixed OXE compatibility cohort has an empty cached train/val split"
            )

        oxe_source_name = str(data_cfg.get("oxe_source_name", "oxe_bridge"))
        train_sources = [(oxe_source_name, oxe_train)]
        val_sources = [(oxe_source_name, oxe_val)]
        ...
        train_dataset = MixedSourceWindowDataset(
            train_sources, mono_sources=(oxe_source_name,)
        )
        val_dataset = MixedSourceWindowDataset(
            val_sources, mono_sources=(oxe_source_name,)
        )
...
def validate_future_value_resume_load(load_result) -> None:
    """Stage 1 may initialize only its newly introduced value-head weights."""
    ...
    bad_missing = [key for key in missing if not key.startswith("future_value_head.")]
...
def validate_stage_transition_preflight(cfg: dict, args: argparse.Namespace) -> str | None:
    ...
    if load_mode not in {"v6_native", "exact", "stage0_to_value"}:
...
def _policy_flow_clean_action(
    ...
    grip_binary = (action_tgt[:, :horizon, 6] > 0.5).float()
...
def _direct_policy_only_forward(
    ...
    *,
    ...
    if multiview_kwargs:
        s = target_model.fuse_views(s, **multiview_kwargs)
...
def compute_policy_flow_matching_loss(
    ...
    L_flow = (
        float(train_cfg.get("policy_flow_pose_weight", 1.0) or 1.0) * pose_mse
        + float(train_cfg.get("policy_flow_grip_weight", 1.0) or 1.0) * grip_mse
    )
...
def main():
    ...
    oxe_representation_weights = LossWeights(**cfg["loss"])
    oxe_representation_weights.action = 0.0
    oxe_representation_weights.idm_reg = 0.0
    ...
    if args.resume is not None and args.resume.exists():
        ...
        if future_value_stage:
            validate_future_value_resume_load(load_res)
    ...
    for epoch in range(start_epoch, cfg["train"]["epochs"]):
        ...
        for batch in tr_loader:
            ...
            oxe_representation_batch = False
            if mixed_source_batch_counts is not None:
                ...
                oxe_representation_batch = bool(
                    train_cfg.get("oxe_representation_only", False)
                    and source_name.startswith("oxe_")
                )
            ...
            if oxe_representation_batch and action_cond is not None:
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                if direct_policy_only:
                    ...
                    policy_kwargs.update(make_policy_flow_training_kwargs(tgt, train_cfg))
                    ...
                    out = _forward_joint_model(
                        ...
                        native_action_no_teacher=(
                            not future_value_only
                            and float(
                                train_cfg.get(
                    ...
                    if future_value_only:
                        ...
                        batch_weights = (
                            oxe_representation_weights
                            if oxe_representation_batch
                            else weights
                        )
                        ...
                        native_action_no_teacher_weight = float(
                            train_cfg.get("native_action_no_teacher_weight", 0.0)
                            or 0.0
                        )
                        if native_action_no_teacher_weight > 0.0:
                            native_action_losses = compute_native_action_losses(
                                {
                                    "pose_norm": out[
                                        "native_action_no_teacher_pose_norm"
                                    ],
                                    "pose": out["native_action_no_teacher_pose"],
                                    "gripper_logit": out[
            ...
            if oxe_representation_batch:
                ...
                if kept_gradients == 0:
                    raise RuntimeError(
                        "OXE representation-only batch produced no permitted gradients; "
            ...
            if measure_step_time:
                ...
                step_duration_window.append(float(local_duration.item()))
            ...
            if rank == 0 and args.print_every and step % args.print_every == 0:
                ...
                native_no_teacher_pose_norm = float(
                    losses.get(
                        "native_no_teacher_L_pose_action_normalized",
                        torch.tensor(0.),
                    ).detach().float()
                )
                native_no_teacher_pose_phys = float(
                    losses.get(
                        "native_no_teacher_L_pose_action_physical",
                        torch.tensor(0.),
                    ).detach().float()
                )
                native_no_teacher_grip = float(
                    losses.get("native_no_teacher_L_grip", torch.tensor(0.)).detach().float()
                )
                ...
                print(f"[rank0] step {step} (ep {epoch}) L_total={float(loss.detach().float()):.4f} "
                      ...
                      f"action={action_loss:.4f} action_pose={action_pose:.4f} "
                      ...
                      f"native_pose_phys={native_no_teacher_pose_phys:.6f} "
        ...
        reached_max_steps = max_steps > 0 and step >= max_steps
        if not reached_max_steps and (epoch + 1) % validate_every_epochs != 0:
            continue
        # Val
        model.eval()
        agg = {}; nb = 0
        ...
        if world > 1:
            ...
            v = torch.tensor([agg.get(kk, 0.0) for kk in keys] + [float(nb)], device=reduce_device)
            dist.all_reduce(v)
        ...
        if rank == 0:
            ...
            val_direct_grip_pos = agg.get("direct_policy_grip_pos_rate", 0.0) / max(1, nb)
            val_direct_grip_prob = agg.get("direct_policy_grip_prob_mean", 0.0) / max(1, nb)
            ...
            val_true_recon = agg.get("true_branch_reconstruction", 0.0) / max(1, nb)
            val_true_match = agg.get("true_branch_matching", 0.0) / max(1, nb)
            val_true_top1 = agg.get("true_branch_matching_top1", 0.0) / max(1, nb)
            val_effect_recon = agg.get("true_effect_reconstruction", 0.0) / max(1, nb)
            val_effect_match = agg.get("true_effect_matching", 0.0) / max(1, nb)
            val_effect_top1 = agg.get("true_effect_matching_top1", 0.0) / max(1, nb)
            val_effect_cos = agg.get("true_effect_cosine", 0.0) / max(1, nb)
            val_value_terminal_bce = agg.get("future_value_terminal_bce", 0.0) / max(1, nb)
            val_value_terminal_acc = agg.get("future_value_terminal_acc", 0.0) / max(1, nb)
            val_value_trajectory_acc = agg.get("future_value_trajectory_acc", 0.0) / max(1, nb)
            val_value_ranking = agg.get("future_value_ranking_loss", 0.0) / max(1, nb)
            val_value_ranking_acc = agg.get("future_value_ranking_acc", 0.0) / max(1, nb)
            val_value_task_swap_l1 = agg.get("future_value_task_swap_logit_l1", 0.0) / max(1, nb)
            print(
