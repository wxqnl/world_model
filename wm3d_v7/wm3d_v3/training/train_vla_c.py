"""VLA Stage-C from-scratch: depth-augmented IDM with explicit native-3D motion signal."""
...
from wm3d_v3.losses import VLALossWeights, compute_losses_vla, _normalize_depth
...
def build_model(cfg: dict) -> JointWorldModelC:
    sc = StateConfig(**cfg["model"]["state"])
    ic = IDMStreamConfig(**cfg["model"]["idm"])
    dc = DepthEncoderConfig(**cfg["model"]["depth_enc"])
    jc = JointCConfig(
        state=sc, idm=ic, depth_enc=dc,
...
def batch_to_device(batch: dict, device: torch.device) -> tuple:
    ...
    return s, c, depth_in, tgt


def compute_losses_c(out: dict, tgt: dict, w: VLALossWeights, w_state_mse: float,
                     w_state_cos: float, w_geom_depth: float) -> dict:
    """VLA loss + world-model anchors (state + depth) for Stage C from-scratch."""
    losses_vla = compute_losses_vla(out, tgt, w)
    L_state_mse = F.mse_loss(out["pred_tokens"].float(), tgt["s_tgt"].float())
    cos = torch.nn.functional.cosine_similarity(
        out["pred_tokens"].float().flatten(-2),
        tgt["s_tgt"].float().flatten(-2), dim=-1).mean()
    L_cos = 1.0 - cos
    L_depth = F.l1_loss(_normalize_depth(out["depth"].float()),
                         _normalize_depth(tgt["depth_tgt"].float()))
    L_total = losses_vla["L_total"] + w_state_mse * L_state_mse + w_state_cos * L_cos + w_geom_depth * L_depth
    losses_vla["L_total"] = L_total
    losses_vla["L_state_mse"] = L_state_mse.detach()
    losses_vla["L_cos"] = L_cos.detach()
    losses_vla["L_geom_depth"] = L_depth.detach()
    return losses_vla
...
def main():
    ...
    if rank == 0:
        ...
        print(f"[rank0] JointWorldModelC: {n_p/1e6:.1f}M trainable params")
    ...
    # split loss config: VLA weights vs world-model weights
    vla_keys = {"action_pose", "action_grip", "aux_idm",
                "grip_pos_weight", "grip_focal_alpha", "grip_focal_gamma", "huber_delta"}
    vla_cfg = {k: v for k, v in cfg["loss"].items() if k in vla_keys}
    weights = VLALossWeights(**vla_cfg)
    w_state_mse = float(cfg["loss"].get("state_mse", 1.0))
    w_state_cos = float(cfg["loss"].get("state_cos", 0.1))
    w_geom_depth = float(cfg["loss"].get("geom_depth", 0.5))
    ...
    for epoch in range(start_epoch, cfg["train"]["epochs"]):
        ...
        for batch in tr_loader:
            if args.max_batches_per_epoch and bi >= args.max_batches_per_epoch:
                break
            s, c, depth_in, tgt = batch_to_device(batch, device)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                out = model(s, c, depth_in=depth_in, pixel=False)
                losses = compute_losses_c(out, tgt, weights,
                                           w_state_mse, w_state_cos, w_geom_depth)
            ...
            if rank == 0 and step % cfg["train"]["log_every"] == 0:
                ...
                print(f"  ep{epoch} step{step} L_total={float(losses['L_total'].detach()):.4f} "
                      f"L_pose={float(losses['L_pose']):.4f} L_state={float(losses['L_state_mse']):.4f} "
                      f"L_depth={float(losses['L_geom_depth']):.4f} gn={float(grad_norm):.2f}")
            step += 1
            bi += 1
        model.eval()
        agg = {}; nb = 0
            ...
            for batch in val_loader:
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    ...
                    losses = compute_losses_c(out, tgt, weights,
                                               w_state_mse, w_state_cos, w_geom_depth)
                for kk, v in losses.items():
                    agg[kk] = agg.get(kk, 0.0) + float(v.detach())
                nb += 1
        if world > 1:
        ...
        if rank == 0:
            ...
            print(f"[rank0] epoch {epoch}: val_total {val_total:.4f} (best {best_val:.4f})")
    if world > 1:
