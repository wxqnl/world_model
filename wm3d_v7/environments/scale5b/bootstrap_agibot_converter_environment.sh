...
CONTRACT_SOURCE="${REPO_ROOT}/environments/scale5b/agibot_converter_environment_contract.json"
...
BUILD_ROOT="${CACHE}/lerobot-${REVISION}-wm3d-v7-build"
POETRY_ENV="${ENV_ROOT}/poetry-1.8.5"
PREPARE_SCRIPT="${REPO_ROOT}/scripts/scale5b/prepare_lerobot_converter_build.py"
...
if [[ -f "${RECEIPT}" && -x "${CONVERTER_PYTHON_BIN}" ]]; then
  "${CONVERTER_PYTHON_BIN}" \
    "${REPO_ROOT}/scripts/scale5b/verify_agibot_converter_environment.py" \
