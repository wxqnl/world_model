...
: "${CONFIG:?Set CONFIG to one materialized V7 5B YAML}"
: "${REPO_ROOT:?Set REPO_ROOT to the wm3d_v7 checkout}"
: "${MASTER_ADDR:?Set MASTER_ADDR}"
: "${MASTER_PORT:?Set MASTER_PORT}"
: "${NNODES:?Set NNODES to 8 or 16}"
...
for path_value in "${CONFIG}" "${REPO_ROOT}" "${PREFLIGHT_REPORT}" "${NODE_LOG}"; do
  if [[ "${path_value}" != /* ]]; then
    echo "Formal V7 5B paths must be absolute: ${path_value}" >&2
    exit 2
  fi
done
...
if [[ "${NNODES}" != "8" && "${NNODES}" != "16" ]]; then
  echo "Formal V7 5B launch requires 8 or 16 eight-GPU nodes" >&2
  exit 2
fi
...
if [[ "${GPU_COUNT}" != "8" ]]; then
  echo "Expected exactly 8 visible GPUs, got ${GPU_COUNT}" >&2
  exit 2
fi
