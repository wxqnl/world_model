...
from wm3d_v3.training.scale5b_checkpoint import Native5BCheckpointManager


SCHEMA = "wm3d_v7_native5b_public_smoke_report_v1"
...
def main() -> None:
    ...
    if (
        raw.get("schema") != "wm3d_v7_native5b_raw_download_receipt_v1"
    ...
    if parameter_count != 4_956_589_929:
        raise ValueError(f"unexpected native 5B parameter count {parameter_count}")

    checkpoint = train_root / "checkpoints" / "step_00000001"
    checkpoint_metadata = Native5BCheckpointManager(checkpoint.parent).verify(checkpoint)
