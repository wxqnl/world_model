#!/usr/bin/env bash
#SBATCH --job-name=wm3d-v7-native5b
#SBATCH --nodes=16
...
: "${REPO_ROOT:?Export REPO_ROOT as the shared wm3d_v7 checkout}"
...
if [[ "${NNODES}" != "8" && "${NNODES}" != "16" ]]; then
  echo "Formal V7 native5b requires 8 or 16 nodes, got ${NNODES}" >&2
  exit 2
fi
...
srun --export=ALL --kill-on-bad-exit=1 --label bash -lc '
  ...
  exec "${REPO_ROOT}/scripts/scale5b/launch_torchrun_node.sh"
'
