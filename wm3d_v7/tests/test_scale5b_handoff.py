...
from scripts.scale5b.preflight_cluster import (
    _parse_ib_rate_gbps,
    _parse_nvlink_topology,
)
from scripts.scale5b.seal_code import DEFAULT_PATTERNS
...
def test_code_seal_covers_native_encoder_dependency_closure() -> None:
    required = {
        "wm3d_v3/__init__.py",
        "wm3d_v3/data/__init__.py",
        "wm3d_v3/encoders/__init__.py",
        "wm3d_v3/encoders/native5b_vggt.py",
        "wm3d_v3/encoders/vggt_encoder.py",
        "wm3d_v3/models/__init__.py",
...
def _materialization_contract() -> DatasetContract:
    return DatasetContract.from_mapping(
        {
            "name": "synthetic_native5b_handoff",
            "feature_fps": 5.0,
            "action_fps": 30.0,
            "T": 24,
            "P": 144,
...
def test_checkpoint_manifest_rejects_path_escape(tmp_path: Path) -> None:
    ...
    with pytest.raises(CheckpointIntegrityError, match="unsafe relative path"):
        Native5BCheckpointManager(checkpoint_root).verify(checkpoint)
