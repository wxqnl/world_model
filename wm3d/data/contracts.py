"""Strict data and lineage contracts for WM3D pretraining.
...
DATASET_SCHEMA = "wm3d_v7_dataset_v1"
SEAL_SCHEMA = "wm3d_v7_dataset_seal_v1"
MANIFEST_SCHEMA = "wm3d_v7_shard_manifest_v1"
...
class DatasetContract:
    ...
    def validate(self) -> None:
        ...
        if not 0.0 < float(self.feature_fps) <= 120.0:
            raise ContractError("feature_fps must be in (0, 120]")
        if not float(self.feature_fps) <= float(self.action_fps) <= 1000.0:
            raise ContractError("action_fps must be in [feature_fps, 1000]")
        ...
        if int(self.T) <= 0 or int(self.K) <= 0 or int(self.P) <= 0:
            raise ContractError("T, P, and K must be positive")
        grid = isqrt(int(self.P))
        if grid * grid != int(self.P):
            raise ContractError("P must describe a square spatial grid")
        if int(self.token_dim) <= 0 or int(self.task_dim) <= 0:
            raise ContractError("token_dim and task_dim must be positive")
        if not 1 <= int(self.num_views) <= 16:
            raise ContractError("num_views must be in [1, 16]")
        if int(self.max_action_groups) <= 0 or int(self.max_action_dim) <= 0:
            raise ContractError("action capacities must be positive")
        if int(self.max_group_id) <= 0 or int(self.max_embodiments) <= 0:
            raise ContractError("group and embodiment capacities must be positive")
        if not 0 <= int(self.max_aux_tokens) <= 64:
            raise ContractError("max_aux_tokens must be in [0, 64]")
        if not 0 < int(self.max_aux_type_id) < int(self.aux_dim):
            raise ContractError("auxiliary type slots must fit inside aux_dim")
...
class DatasetSeal:
    ...
    def validate(self) -> None:
        ...
        if self.dataset_schema != DATASET_SCHEMA:
            raise ContractError("seal dataset schema mismatch")
