"""Transactional, sharded, exact-resume checkpoints for WM3D.
...
class DistributedCheckpointManager:
    ...
    def load_model_warmstart(
        ...
        *,
        ...
        """Integrity-check and partially load a predecessor model checkpoint.

        Warmstart is intentionally model-only: optimizer, RNG and sampler state
        belong to the new run. Every current parameter must exist in the source
        unless its canonical name is under an explicitly declared new prefix.
        """
        ...
        try:
            ...
            if not matched:
                raise CheckpointIntegrityError("warmstart loaded no shared model tensors")
            load_state = {
