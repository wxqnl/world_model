...
def build_direct_vggt_teacher(
    ...
    *,
    ...
    if (
        geometry_grid * geometry_grid != int(model["P"])
        or appearance_grid * appearance_grid != int(model["appearance_P"])
    ):
        raise ValueError("direct VGGT P values must be square grids")
    encoder = replace(
        base,
        token_grid=geometry_grid,
        appearance_token_grid=appearance_grid,
        appearance_feature_layer=int(
            closure["appearance_feature_layer"]
        ),
    ...
    return DirectVGGTTeacherAdapter(
        DirectVGGTTeacherConfig(
            ...
            appearance_context_frames=int(
                model["appearance_context_frames"]
            ),
