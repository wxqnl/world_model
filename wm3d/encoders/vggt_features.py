"""Leak-free three-view VGGT cache encoder for WM3D."""
...
class VGGTFeatureEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if int(token_grid) != 12:
            raise ValueError("wm3d production token grid must be 12")
    ...
    def forward(
        ...
        if images.ndim != 6:
            raise ValueError("wm3d images must be [B,T,V,3,H,W]")
        batch, times, views, channels, height, width = images.shape
        if views != 3 or channels != 3:
            raise ValueError("wm3d requires exactly three RGB views")
        ...
        if encoded.get("geom_extra_missing"):
            raise RuntimeError(
                "formal wm3d requires all VGGT geometry heads: "
