...
目标和当前 native-direct RGB 结构，但不再创建或读取 episode 级 VGGT latent cache、
...
4. 同一 frozen VGGT 另以 anchor camera 的完整 `T+K` 顺序构造 target-only P64（1B）/P144（5B）
   teacher，只截取未来 K 个位置作为监督标签；
5. 因果 native observation 进入 factual 3D/运动主干并直接驱动原始 V7 RGB decoder；
...
- P64/P144 factual geometry/运动主干、原始 V7 RGB decoder 与受限晚期高频 refiner。
...
5B 的路径填写、环境、数据准备、资格和正式训练命令统一见
[5B 训练](WM3D_5B_SCALING.md)。不要使用旧 site 或 transport/P256 runtime。

5B 与 1B 共用实现，空间 grid 为 P144，T24/K16，RGB 为 384px；
不提取 absolute future P256，也没有单独的 5B cache 实现。

## 关键参数

- `DIRECT_ENCODE_CHUNK_ROWS=32`：VGGT 每次编码的时空行数；encoder OOM 时自动
  减半，最低到 `DIRECT_MINIMUM_CHUNK_ROWS`。
- `DIRECT_PREFETCH_WINDOWS=32`：新站点默认的有界 uint8 RGB window 队列，不是 latent cache。
- `DIRECT_PREFETCH_WORKERS=4`：新站点默认的 CPU 预取并行度；batch coalescing 和有界队列
  控制重复解码与内存。并行度应结合实测 CPU、磁盘与等待时间检查，不能无限增大。
- `DIRECT_VIDEO_INDEX_CACHE_ASSETS=128`：只缓存小型 PTS 数组，不缓存 RGB 或
  latent。
- `DIRECT_DECODE_WORKERS=1`：每个 rank 顺序解码相机；多 rank 已经提供节点级并行，
  避免嵌套线程争抢 CPU 和视频盘。
- `DIRECT_PREPARED_ROW_CACHE_GIB_PER_RANK=8`：只在内存中保留近期完成 resize 的
  uint8 相机行；相邻 window 重用后立即按字节 LRU 淘汰，不写视觉 latent。
- `DIRECT_APPEARANCE_FEATURE_LAYER=-1`：当前 native-direct 禁用 absolute-P256 appearance 提取；
...
- 历史 direct 验证的 geometry、RGB、depth、point、camera 均有限；当前关闭 appearance 提取；
- 真实 H100 上 target-only `T+K` P64 teacher 的 BF16 输出有限，按等价图像预算
  批处理并在 OOM 时二分回退；
- 历史两卡 1.327B FSDP2 已完成 objective、backward、optimizer、保存与恢复；
  这项旧证据不能替代当前 5B 的实机资格；
