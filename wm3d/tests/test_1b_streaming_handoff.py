...
def test_1b_streaming_presets_use_native_1b_and_saturating_batch() -> None:
    model = yaml.safe_load((ROOT / "configs/model/native_1b.yaml").read_text())
    validate_model_profile(model)
    assert model["expected_parameter_count"] == 1_319_203_443
...
def test_1b_site_init_and_plan_are_scale_specific(tmp_path: Path) -> None:
    ...
    assert "MODEL_PROFILE=configs/model/native_1b.yaml" in payload
    assert "ENCODER_CONTRACT=configs/encoder/vggt_native_p64.yaml" in payload
...
def test_1b_oxe_mix_is_60_sources_without_agibot_or_pca() -> None:
    ...
    representation = _representation_from_profiles(
        ...
        model_path=ROOT / "configs/model/native_1b.yaml",
