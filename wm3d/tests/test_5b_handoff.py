...
def test_5b_shell_gate_matches_published_model_and_objective() -> None:
    from scripts.cluster.check_5b_contract import validate_built_model

    wrapper = (ROOT / "scripts/cluster/wm3d_5b.sh").read_text()
    assert 'scripts/cluster/check_5b_contract.py' in wrapper
    profile = yaml.safe_load(
        (ROOT / "configs/model/native_5b_v8_native_direct_rgb.yaml").read_text()
    )
    with torch.device("meta"):
        model = build_world_model(profile)
    validate_built_model(model)
...
def test_5b_presets_match_action_owned_5b_and_64_h200(
    ...
    assert objective["objective"]["context_pixel_action_motion_threshold"] == 0.03
    assert objective["objective"]["context_pixel_action_motion_gain"] == 4.0
    assert objective["objective"]["context_pixel_action_negative_min_distance"] == 0.05
