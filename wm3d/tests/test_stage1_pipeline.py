...
import numpy as np
import torch
import wm3d.stage1_planner.rollout_audit as rollout_audit_contract
import wm3d.stage1_planner.replay_authority as replay_authority_contract
import wm3d.stage1_planner.train as stage1_train

from scripts.eval_stage1 import (
    _ACTION_FIELDS,
    _LEARNED_FIELDS,
...
from wm3d.data.unified_cache_dataset import CacheDataError, _active_source_names
from wm3d.stage1_planner.candidates import deterministic_action_cost
from wm3d.stage1_planner.dataset import (
    BRANCH_INDEX_SCHEMA,
    BRANCH_SCHEMA,
    BRANCH_SEAL_SCHEMA,
...
from wm3d.stage1_planner.losses import planner_loss
from wm3d.stage1_planner.planner_head import NativePlannerConfig, NativePlannerHead, planning_score
from wm3d.stage1_planner.rollout import single_horizon_native_rollout
from wm3d.stage1_planner.system import NativePlanningSystem, Stage1SystemConfig
from wm3d.stage1_planner.train import (
    _expectations,
    _load_stage1,
...
from scripts.materialize_stage1_branches import (
...
from scripts.produce_robocasa_stage1_candidates import (
...
from wm3d.data.manifest_contract import canonical_sha256
from wm3d.stage1_planner.rollout_audit import (
...
from wm3d.stage1_planner.replay_authority import (
...
from wm3d.training.launch_qualification import LaunchQualificationError
...
def test_replay_environment_identity_tamper_is_rejected(
    ...
    paths = {
        ...
        "v7_action_contract": execution / "wm3d/data/v7_action_contract.py",
        "v7_contracts": execution / "wm3d/data/v7_contracts.py",
        "action_bridge": execution / "wm3d/stage1_planner/action_bridge.py",
    }
...
def test_rollout_audit_loader_rejects_symlink_and_referent_tamper(
    ...
    monkeypatch.setattr(
        "wm3d.stage1_planner.replay_authority.load_replay_authority",
...
def test_stage1_release_manual_tracks_required_audit_cli_and_schemas() -> None:
    manual = (
        Path(__file__).resolve().parents[1]
        / "docs"
        / "WM3D_STAGE1_UNIFIED.md"
    ).read_text(encoding="utf-8")
    audit_block = manual.split(
        "./run_wm3d.sh stage1-audit-rollouts \\", 1
    )[1].split("```", 1)[0]
    replay_block = manual.split(
        "./run_wm3d.sh stage1-replay-authority \\", 1
    )[1].split("```", 1)[0]
...
def test_stage1_checkout_uses_shared_clean_runtime_authority(
    ...
    monkeypatch.setattr(
        "wm3d.stage1_planner.train.verify_clean_runtime_checkout", fake
    )
...
def test_stage1_checkpoint_payload_loads_follow_launch_qualification() -> None:
    ...
    eval_source = Path(__file__).parents[1] / "scripts" / "eval_stage1.py"
...
def _rollout_batch() -> dict[str, torch.Tensor]:
    ...
    batch={
        ...
        "action_normalization_scale":torch.ones(B,G,A),"state_normalization_offset":torch.zeros(B,G,6),
        "state_normalization_scale":torch.ones(B,G,6),"aux_values":torch.zeros(B,T,2,4),
