"""Native world/action objectives for WM3D.
...
class WM3DLossConfig:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, float]) -> "WM3DLossConfig":
...
def _validate_finite(name: str, value: torch.Tensor) -> None:
    if not bool(torch.isfinite(value).all()):
        raise FloatingPointError(f"non-finite wm3d loss input: {name}")
...
def wm3d_loss(
    ...
    if missing_output or missing_batch:
        raise KeyError(
            f"wm3d loss fields missing output={sorted(missing_output)} "
