"""Formal WM3D pretraining entrypoint.

Launch only through torchrun/Slurm wrappers in ``scripts``.  The
...
from wm3d.training.loss import (
    WM3DLossConfig,
    wm3d_loss,
)
...
RUN_CONTRACT_SCHEMA = "wm3d_v7_run_contract_v1"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--preflight-only",
        action="store_true",
        help="Validate config, data and hardware without constructing the model.",
    )
...
def _validate_config(
    ...
    *,
    ...
    if int(dist_cfg["shard_degree"]) <= 1:
        raise RuntimeContractError("formal wm3d requires FSDP2 sharding")
...
def main() -> None:
    ...
    try:
        ...
        boundary_paths = [
            ...
            repo_root / "wm3d/models/wm3d.py",
            repo_root / "wm3d/data/dataset.py",
            repo_root / "wm3d/data/sampler.py",
            repo_root / "wm3d/training/loss.py",
            repo_root / "wm3d/training/runtime.py",
            repo_root / "wm3d/training/checkpoint.py",
            repo_root / "wm3d/training/config.py",
            repo_root / "wm3d/training/environment.py",
