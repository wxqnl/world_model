"""Distributed runtime primitives for WM3D."""
...
from wm3d.models.wm3d import WM3D
...
def initialize_distributed(timeout_minutes: int = 30) -> DistributedContext:
    ...
    if not torch.cuda.is_available():
        raise RuntimeContractError("wm3d formal training requires CUDA")
...
def apply_fsdp2(
    ...
    *,
    ...
    """Bottom-up FSDP2/HSDP wrapping for the WM3D core."""
    ...
    if len(units) != (
        ...
        raise RuntimeContractError("wm3d transformer-unit enumeration drifted")
    seen: set[int] = set()
    for unit in units:
        if id(unit) in seen:
    ...
    if not isinstance(model, FSDPModule):
        raise RuntimeContractError("wm3d root did not become an FSDPModule")
    return mesh


def set_gradient_sync(model: WM3D, enabled: bool) -> None:
    if not isinstance(model, FSDPModule):
...
def initialize_adamw_state(optimizer: torch.optim.AdamW) -> None:
    ...
    for group in optimizer.param_groups:
        if group.get("amsgrad", False):
            raise RuntimeContractError("wm3d AdamW does not support AMSGrad")
        if group.get("capturable", False) or group.get("fused", False):
            raise RuntimeContractError(
                "wm3d eager AdamW state expects capturable=fused=False"
            )
...
def assert_dependency_boundary(paths: Iterable[Path]) -> None:
    ...
    if violations:
        raise RuntimeContractError(
            "WM3D dependency boundary failed:\n" + "\n".join(violations)
        )
...
def verify_parameter_budget(
    ...
    required = {"minimum_parameters", "maximum_parameters", "expected_parameters"}
    missing = required.difference(budget)
    if missing:
        raise RuntimeContractError(f"model_budget misses {sorted(missing)}")
    minimum = int(budget["minimum_parameters"])
    maximum = int(budget["maximum_parameters"])
    expected = int(budget["expected_parameters"])
    if not 0 < minimum <= expected <= maximum:
        raise RuntimeContractError("invalid model_budget bounds")
    counts = model.parameter_counts()
    total = counts["total"]
    if not minimum <= total <= maximum:
        raise RuntimeContractError(
            f"wm3d parameter count {total} outside [{minimum}, {maximum}]"
        )
    if total != expected:
        raise RuntimeContractError(
            f"wm3d parameter count {total} does not equal expected {expected}"
        )
    return counts
