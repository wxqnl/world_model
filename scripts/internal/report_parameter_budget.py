...
from wm3d.data.contracts import resolve_regular_file
...
def _report(config_path: Path, expected: int | None) -> dict[str, object]:
    path = resolve_regular_file(config_path.parent, config_path.name)
    raw = yaml.safe_load(path.read_text(encoding="utf-8"))
    configured_expected = int(raw["model_budget"]["expected_parameters"])
    if expected is None:
        expected = configured_expected
    elif expected != configured_expected:
        raise ValueError(
            f"命令行期望参数 {expected:,} 与配置 {configured_expected:,} 不一致"
        )
