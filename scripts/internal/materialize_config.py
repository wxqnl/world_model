#!/usr/bin/env python3
"""Bind a WM3D template to one sealed dataset/code/topology."""
...
def main() -> None:
    ...
    allowed_world_sizes = tuple(int(value) for value in hardware.get("allowed_world_sizes", ()))
    gpus_per_node = int(hardware.get("gpus_per_node", 0))
    if args.world_size <= 0 or args.world_size not in allowed_world_sizes:
        raise ValueError(
            f"world-size {args.world_size} is not allowed by {allowed_world_sizes}"
        )
    if gpus_per_node <= 0 or args.world_size % gpus_per_node:
        raise ValueError("world-size is incompatible with hardware.gpus_per_node")
    ...
    model_representation = tuple(config["model"][key] for key in representation_fields)
    data_representation = tuple(getattr(contract, key) for key in representation_fields)
    if model_representation != data_representation:
        raise ValueError(
            "model/data representation mismatch: "
            f"model={model_representation}, data={data_representation}"
        )
