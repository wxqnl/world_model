...
from wm3d.data.contracts import atomic_write_json
...
class Pipeline:
    ...
    def doctor(self) -> None:
        self.banner("doctor")
        gpus_per_node = _integer("TRAIN_GPUS_PER_NODE")
        shard_degree = _integer("SHARD_DEGREE", minimum=2)
        if self.world_size % shard_degree:
            raise PipelineError("world size 必须能被 SHARD_DEGREE 整除")
        for template in (self._template(canary=True), self._template(canary=False)):
            ...
            if self.world_size not in allowed:
                raise PipelineError(
                    f"world size {self.world_size} 不在配置允许范围 {allowed}"
                )
        ...
        report = {
            ...
            "architecture": "WM3D native3d",
        }
        print(json.dumps(report, ensure_ascii=False, sort_keys=True))

    def lock(self) -> None:
        self.banner("冻结上游版本")
        template_path = self.repo / "configs/data/raw_sources.lock.yaml"
    ...
    def _prepare_assets(self) -> None:
        ...
        self.command(
            [
                ...
                _environment("VGGT_SOURCE_ARCHIVE_SHA256"),
                "--vggt-source-tree-sha256",
                _environment("VGGT_SOURCE_TREE_SHA256"),
                "--vggt-model-revision",
    ...
    def _materialize(self, *, canary: bool) -> tuple[Path, Path]:
        ...
        self.python_command(
            ...
            self.repo / "environments/environment_contract.json",
            "--environment-receipt",
    ...
    def _training_job(self, config: Path, run_root: Path, *, wait: bool) -> str:
        ...
        rendezvous = f"wm3d-{kind}-{self._lineage(run_root.name)[:16]}"
    ...
    def train(self) -> None:
        ...
        if not self.dry_run:
            ...
            atomic_write_json(
                receipt,
                {
                    "schema": "wm3d_v7_slurm_submission_v1",
                    "job_id": job_id,
                    "config": str(config),
...
if __name__ == "__main__":
    try:
        ...
        print(f"WM3D pipeline FAIL: {error}", file=sys.stderr)
