...
def _window_config(data_cfg: dict) -> WindowConfig:
    return WindowConfig(T=data_cfg["T"], k=data_cfg["k"],
                        ...
                        window_geom_subdir=str(data_cfg.get("window_geom_subdir", "vggt_window_geom_p64")),
...
def batch_to_device(batch: dict, device: torch.device, k: int, direct_policy_only: bool = False) -> tuple:
    ...
    if not direct_policy_only:
        ...
        if "point_tgt" in batch:
            tgt["point_tgt"] = batch["point_tgt"].to(device, non_blocking=True)
        if "point_conf_tgt" in batch:
            tgt["point_conf_tgt"] = batch["point_conf_tgt"].to(device, non_blocking=True)
        if "pose_geom_tgt" in batch:
            tgt["pose_geom_tgt"] = batch["pose_geom_tgt"].to(device, non_blocking=True)
        if "pose_geom_conf_tgt" in batch:
            tgt["pose_geom_conf_tgt"] = batch["pose_geom_conf_tgt"].to(device, non_blocking=True)
        if "depth_conf_tgt" in batch:
            tgt["depth_conf_tgt"] = batch["depth_conf_tgt"].to(device, non_blocking=True)
...
def compute_evaluator_pairwise_loss(
    ...
    *,
    ...
    for variant in variants:
        ...
        var_out = model(s, c, action_cond=var_cond, pixel=False, bridging=False)
        ...
        for batch in tr_loader:
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                if direct_policy_only:
                    ...
                    out = target_model.action_policy(s, task_emb=c)
                    ...
                    out = model(s_cond, c_cond, action_cond=action_cond_model, context_rgb=context_rgb_cond,
                                prior_clean_tokens=prior_clean_tokens,
                                pixel=do_pixel, bridging=False) \
                        if isinstance(model, DDP) else model(s_cond, c_cond, action_cond=action_cond_model,
            ...
            if rank == 0 and args.print_every and step % args.print_every == 0:
                ...
                depth_tmp = float(losses.get("L_depth_temporal", torch.tensor(0.)).detach().float())
                point = float(losses.get("L_point", torch.tensor(0.)).detach().float())
                point_tmp = float(losses.get("L_point_temporal", torch.tensor(0.)).detach().float())
                pose_g = float(losses.get("L_pose_geom", torch.tensor(0.)).detach().float())
                print(f"[rank0] step {step} (ep {epoch}) L_total={float(loss.detach().float()):.4f} "
                      f"rgb_L1={rgb_l1:.4f} lpips={lpv:.4f} "
                      f"depth={depth:.4f} depth_ch={depth_ch:.4f} depth_mo={depth_mo:.4f} depth_tmp={depth_tmp:.4f} "
                      f"point={point:.4f} point_tmp={point_tmp:.4f} pose_g={pose_g:.4f} "
