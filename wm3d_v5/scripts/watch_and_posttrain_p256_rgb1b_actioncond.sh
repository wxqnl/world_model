...
CFG="${CFG:-$PROJ/configs/v3_p256_rgb1b_actioncond_oxe.yaml}"
OUT_ROOT="${OUT_ROOT:-$PROJ/results/wm3d_v3_p256_rgb1b_actioncond}"
CKPT="${CKPT:-$OUT_ROOT/ckpt/best.pt}"
FINAL_CKPT="${FINAL_CKPT:-$OUT_ROOT/ckpt/epoch_019.pt}"
TRAIN_PID_FILE="${TRAIN_PID_FILE:-/data/Minko/logs/train_v3_p256_rgb1b_actioncond_latest.pid}"
TRAIN_LOG="${TRAIN_LOG:-/data/Minko/logs/train_v3_p256_rgb1b_actioncond_latest.log}"
POLL_SECONDS="${POLL_SECONDS:-300}"

RUN_ID="${RUN_ID:-actioncond_$(date +%Y%m%d_%H%M%S)}"
WATCH_LOG_DIR="${WATCH_LOG_DIR:-/data/Minko/logs}"
WATCH_LOG="$WATCH_LOG_DIR/posttrain_p256_rgb1b_actioncond_watcher_${RUN_ID}.log"

mkdir -p "$WATCH_LOG_DIR"
exec > >(tee -a "$WATCH_LOG") 2>&1

echo "[$(date)] action-conditioned watcher started"
echo "root=$ROOT"
echo "proj=$PROJ"
echo "cfg=$CFG"
...
if [[ -n "$train_pid" ]]; then
  ...
  while pgrep -f "python.*-m wm3d_v3.training.train --cfg .*v3_p256_rgb1b_actioncond_oxe.yaml" >/dev/null; do
    tail -n 5 "$TRAIN_LOG" 2>/dev/null || true
    sleep "$POLL_SECONDS"
  done
fi
