...
def adapter_forward(adapter, wm_out: dict, context_rgb: torch.Tensor, action_cond: torch.Tensor, c: torch.Tensor, target_latents: torch.Tensor):
    return adapter(
        wm_out["pred_tokens"],
        wm_out["depth"],
    ...
    with torch.no_grad():
        for bi, batch in enumerate(loader):
            ...
            with torch.autocast(device_type="cuda", dtype=autocast_dtype, enabled=device.type == "cuda"):
                ...
                delta_latents = adapter_forward(adapter, wm_out, context_rgb, action_cond, c, target_latents)
...
def main() -> None:
    ...
    adapter_cfg = HunyuanLatentAdapterConfig(hidden=args.hidden, n_blocks=args.n_blocks)
    adapter = HunyuanLatentAdapter(adapter_cfg).to(device)
    if args.residual_from_rough:
        adapter.zero_init_output()
    if world > 1:
        adapter = DDP(adapter, device_ids=[local])
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            with torch.autocast(device_type="cuda", dtype=autocast_dtype, enabled=device.type == "cuda"):
                ...
                if args.decoded_l1_weight > 0 or args.decoded_motion_l1_weight > 0:
                    decoded = decode_hunyuan_latents_grad(vae, pred_latents)
        ...
        if rank == 0:
            ...
            if score < best:
                ...
                # Small decoded demo from rank0's current validation loader.
                try:
