...
WATCH_LOG="${WATCH_LOG:-${LOG_DIR}/watch_140m_depth_stabilized_4node_test_flow_v1.log}"

STAGE0_RUN="train_140m_stage0_visual_depth_stabilized_4node_v1"
STAGE0_CFG="configs/v3_p64_140m_stage0_visual_depth_stabilized_4node_v1.yaml"
STAGE0_ROOT="results/wm3d_v3_p64_140m_stage0_visual_depth_stabilized_4node_v1"

STAGE1_RUN="train_140m_stage1_dynamics_visual_replay_depth_stabilized_4node_v1"
STAGE1_CFG="configs/v3_p64_140m_stage1_dynamics_visual_replay_depth_stabilized_4node_v1.yaml"
STAGE1_ROOT="results/wm3d_v3_p64_140m_stage1_dynamics_visual_replay_depth_stabilized_4node_v1"

STAGE1P5_RUN="train_140m_stage1p5_hunyuan_bridge_depth_stabilized_4node_v1"
STAGE1P5_CFG="configs/v3_p64_140m_stage1p5_hunyuan_bridge_depth_stabilized_4node_v1.yaml"
STAGE1P5_ROOT="results/wm3d_v3_p64_140m_stage1p5_hunyuan_bridge_depth_stabilized_4node_v1"

STAGE2_RUN="train_140m_stage2_action_scaffold_depth_stabilized_4node_v1"
STAGE2_CFG="configs/v3_p64_140m_stage2_action_scaffold_depth_stabilized_4node_v1.yaml"
STAGE2_ROOT="results/wm3d_v3_p64_140m_stage2_action_scaffold_depth_stabilized_4node_v1"
...
any_worker_alive() {
  ...
  for host in "${WORKER_HOSTS[@]}"; do
    if worker_pid_alive "${run}" "${rank}" "${host}"; then
      return 0
    fi
    rank=$((rank + 1))
  done
...
launch_stage() {
  ...
  if [[ -n "${resume}" ]]; then
    SYNC_CACHE="${SYNC_CACHE}" MANIFEST="manifests/oxe_droid20k_depthplus_world_v1.jsonl" CFG="${cfg_name}" RUN_NAME="${run}" MASTER_PORT="${port}" RESUME_CKPT="${resume}" \
      bash scripts/run_300m_stage_4node_v1.sh >> "${WATCH_LOG}" 2>&1
  else
    SYNC_CACHE="${SYNC_CACHE}" MANIFEST="manifests/oxe_droid20k_depthplus_world_v1.jsonl" CFG="${cfg_name}" RUN_NAME="${run}" MASTER_PORT="${port}" \
      bash scripts/run_300m_stage_4node_v1.sh >> "${WATCH_LOG}" 2>&1
  fi
}
...
wait_stage() {
  ...
  if any_worker_alive "${run}"; then
    log "waiting_${run}_workers"
    while any_worker_alive "${run}"; do
      sleep 30
    done
  fi
...
eval_stage() {
  ...
  if [[ "${RUN_NATIVE3D}" == "1" ]]; then
    ...
    GPU=0 CFG="${cfg}" CKPT="${ckpt}" OUT_DIR="${out_dir}/native3d_benchmark_v2" \
      MAX_BATCHES_PER_DATASET=2 N_VIZ=6 \
...
log "flow_start 140m depth-stabilized 4node test stage0->stage2"

if pid_alive "${LOG_DIR}/${STAGE0_RUN}_node0.pid" || any_worker_alive "${STAGE0_RUN}"; then
  log "stage0_already_running; will wait"
elif best_or_latest "${STAGE0_ROOT}" >/dev/null 2>&1; then
  log "stage0_ckpt_already_exists; skip_launch"
else
  launch_stage "${STAGE0_RUN}" "${STAGE0_CFG}" 29740
fi
...
launch_stage "${STAGE1_RUN}" "${STAGE1_CFG}" 29741 "${STAGE0_CKPT}"
...
launch_stage "${STAGE1P5_RUN}" "${STAGE1P5_CFG}" 29742 "${STAGE1_CKPT}"
...
launch_stage "${STAGE2_RUN}" "${STAGE2_CFG}" 29743 "${STAGE1P5_CKPT}"
