...
def _tiny_config(*, activation_checkpointing: bool = False) -> WM3DConfig:
    return WM3DConfig(
        T=3,
        P=4,
        K=2,
...
def test_5b_preset_parameter_budget_is_frozen() -> None:
...
def test_multiview_forward_backward_and_native_losses(checkpointing: bool) -> None:
    torch.manual_seed(4)
    model = WM3D(_tiny_config(activation_checkpointing=checkpointing))
    inputs = _tiny_inputs()
    output = model(**inputs)
    ...
    losses = wm3d_loss(output, batch, WM3DLossConfig())
...
def test_dependency_guard_rejects_later_architecture(tmp_path: Path) -> None:
    ...
    good.write_text(
        "schema: wm3d_v7_dataset_v1\nsha256: 0123456789a2abcdef\n",
        encoding="utf-8",
    )
    assert_dependency_boundary([good])
